...
import com.atomikos.icatch.imp.DefaultCompositeTransactionManager;
import com.atomikos.icatch.imp.DefaultTransactionService;
...
public class AssemblerImp implements Assembler {
	...
	public TransactionServiceProvider assembleTransactionService() {
		...
		if (configProperties.getFeatureFlag("207056")) {
			return new DefaultTransactionService(tmUniqueName, recoveryManager, idMgr, maxTimeout, maxActives, true, recoveryLog);
		} else {			
	...
	public CompositeTransactionManager assembleCompositeTransactionManager() {
		if (Configuration.getConfigProperties().getFeatureFlag("207056")) {
			return new DefaultCompositeTransactionManager();
		} else {			
