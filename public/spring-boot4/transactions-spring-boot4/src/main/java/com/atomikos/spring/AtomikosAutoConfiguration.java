...
@AutoConfigureBefore(
        name = { "org.springframework.boot.transaction.jta.autoconfigure.JtaAutoConfiguration",
...
public class AtomikosAutoConfiguration {
    ...
    @ConditionalOnProperty(prefix = "spring.jta.atomikos.datasource", name="enable-pool", havingValue = "false",  matchIfMissing = false)
    AtomikosNonPoolingXADataSourceWrapper xaNonPoolingDataSourceWrapper() {
        return new AtomikosNonPoolingXADataSourceWrapper();
    }
	
    @Bean
    @ConditionalOnMissingBean(XADataSourceWrapper.class)
    @ConditionalOnProperty(prefix = "spring.jta.atomikos.datasource", name="enable-pool", havingValue = "true",  matchIfMissing = true)
    ...
    JtaTransactionManager transactionManager(UserTransaction userTransaction, TransactionManager transactionManager,
        ...
        transactionManagerCustomizers.ifAvailable((customizers) -> customizers.customize((org.springframework.transaction.TransactionManager)jtaTransactionManager));
