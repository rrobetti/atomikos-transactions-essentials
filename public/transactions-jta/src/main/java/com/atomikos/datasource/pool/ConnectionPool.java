...
public abstract class ConnectionPool<ConnectionType> implements XPooledConnectionEventListener<ConnectionType>
{
	...
	private synchronized ConnectionType growPool(long remainingTime) throws CreateConnectionException {
		ConnectionType ret = null;
		if (canGrow()) { // cf case 181871 
			...
			XPooledConnection<ConnectionType> xpc = null;
			try {
				xpc = futureXPC.get(remainingTime, TimeUnit.MILLISECONDS);
	...
	private ConnectionType createConnectionProxy(XPooledConnection<ConnectionType> xpc) throws CreateConnectionException {
		ConnectionType ret = null;
		try {
